...
/*   Updated: 2018/07/26 12:05:40 by fchevrey         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			switch_light(GtkWidget *widget, gboolean state, gpointer param)
{
	...
	{
		g_data->lights[g_ui->page_light].disabled = 1;
		--g_data->nb_lights_on;
	}
}
...
void			add_one_light(GtkWidget *widget, gpointer param)
{
	...
	if (!(lights_new = (t_light*)malloc(sizeof(t_light) * g_data->nb_lights)))
		return ;
...
void			change_light_angle(GtkWidget *widget, gpointer param)
{
	...
	dir = yaw(dir, *vec);
	dir = roll(dir, *vec);
	g_data->lights[g_ui->page_light].dir = dir;
}
