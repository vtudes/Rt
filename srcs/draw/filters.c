...
/*   Updated: 2018/07/26 12:25:10 by mlantonn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_color	sepia2(t_data *data, t_point crd)
{
	t_color	color;
	t_added	sepia;
...
void			sepia(t_data *data)
{
	...
	while (++crd.y < WIN_H)
	{
		...
		{
			if (data->clust_i == CLUST_CLIENT)
				data->cimg[(int)(crd.x + crd.y * WIN_W)] = sepia2(data, crd).c;
			else
				data->img->pxl[(int)(crd.x + crd.y \
					* WIN_W)] = sepia2(data, crd).c;
		}
	}
}
