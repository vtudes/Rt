...
/*   Updated: 2018/07/26 12:20:53 by mlantonn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "draw.h"

static t_color	get_color(t_data *data, t_vec ray, t_inter inter)
{
	t_color color;
		...
		while (++px.x < g_data->px)
		{
			if (g_data->clust_i == CLUST_CLIENT)
			{
				if (crd.x + px.x >= 0 && crd.x + px.x < WIN_W && \
					...
					g_data->cimg[crd.x + px.x + (crd.y + px.y) \
						* WIN_W] = get_color(data, ray, inter).c;
			}
			else
				pt_to_pixelbuf((t_point){crd.x + px.x, crd.y + px.y}, \
					data->img, get_color(data, ray, inter).c);
		}
	}
}
