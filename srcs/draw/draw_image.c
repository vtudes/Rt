...
/*   Updated: 2018/07/26 12:15:09 by mlantonn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_color	draw_px(t_data *data, t_vec vp)
{
	t_inter		inter;
	t_vec		ray;
	t_color		ret;
	...
	while (++crd.y < ymax)
	{
		...
		while (++crd.x < xmax + 1)
		{
			...
			if (g_data->clust_i == CLUST_CLIENT)
				g_data->cimg[crd.x + (crd.y * WIN_W)] = draw_px(g_data, vp).c;
			else
				pt_to_pixelbuf(crd, g_data->img, draw_px(g_data, vp).c);
		}
	}
	pthread_exit(NULL);
}
